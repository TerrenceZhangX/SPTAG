...
// 4) Dead-then-rejoin: once a node is presumed Dead, an inbound ping
//    bearing a higher incarnation refutes the death and brings it back.
BOOST_AUTO_TEST_CASE(DeadThenRejoinViaIncarnationRefute)
{
    ...
    // Partition. A will eventually decide B is Dead.
    mesh.Drop(0, 1);
    mesh.Pump(8);
    BOOST_REQUIRE_EQUAL(static_cast<int>(HealthOf(a, 1)),
                        static_cast<int>(MemberHealth::Dead));

    // Heal the partition. B is alive; gossip from B (with B's own
    // incarnation 0 self-state) should refute A's Dead view via the
    // standard ApplyUpdates(Self refute) path on B's side, but more
    // importantly direct pings/acks from B should flip A back to Alive.
    mesh.dropped.clear();
    mesh.Pump(6);
