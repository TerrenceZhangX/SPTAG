...
//
// Spec:  tasks/distributed-index-scale-2k/design-docs/ft-fault-cases/posting-router-owner-failover.md
...
// PD + TiKV brought up by the matching posting-router-owner-failover_test.sh harness.
//
// Multi-node and chaos sub-cases are guarded by env vars and skipped
// unless the harness exports them; see posting-router-owner-failover_test.sh.
//
// The 1M-scale perf gate (mandatory before closure) lives in
// posting-router-owner-failover_perf.sh. Unit repro and perf gate are independent; both
...
std::shared_ptr<TiKVIO> MakeIO(const std::string& tag) {
    ...
    std::string prefix = "fault_posting-router-owner-failover_" + tag + "_" + std::to_string(now) + "_";
...
//
//   if (std::getenv("SPTAG_FAULT_POSTING_ROUTER_OWNER_FAILOVER") != nullptr) { /* armed */ }
//
// The harness sets SPTAG_FAULT_POSTING_ROUTER_OWNER_FAILOVER=1 for the "fault" perf tier
