...
// PD + TiKV brought up by the matching online-insert-path-b-timeout-but-applied_test.sh harness.
//
// Multi-node and chaos sub-cases are guarded by env vars and skipped
// unless the harness exports them; see online-insert-path-b-timeout-but-applied_test.sh.
//
// The 1M-scale perf gate (mandatory before closure) lives in
// online-insert-path-b-timeout-but-applied_perf.sh. Unit repro and perf gate are independent; both
...
std::shared_ptr<TiKVIO> MakeIO(const std::string& tag) {
    ...
    std::string prefix = "fault_online-insert-path-b-timeout-but-applied_" + tag + "_" + std::to_string(now) + "_";
...
//
//   if (std::getenv("SPTAG_FAULT_ONLINE_INSERT_PATH_B_TIMEOUT_BUT_APPLIED") != nullptr) { /* armed */ }
//
// The harness sets SPTAG_FAULT_ONLINE_INSERT_PATH_B_TIMEOUT_BUT_APPLIED=1 for the "fault" perf tier
