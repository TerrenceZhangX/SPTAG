...
// PD + TiKV brought up by the matching online-insert-idempotency-on-retry_test.sh harness.
//
// Multi-node and chaos sub-cases are guarded by env vars and skipped
// unless the harness exports them; see online-insert-idempotency-on-retry_test.sh.
//
// The 1M-scale perf gate (mandatory before closure) lives in
// online-insert-idempotency-on-retry_perf.sh. Unit repro and perf gate are independent; both
...
std::shared_ptr<TiKVIO> MakeIO(const std::string& tag) {
    ...
    std::string prefix = "fault_online-insert-idempotency-on-retry_" + tag + "_" + std::to_string(now) + "_";
...
//
//   if (std::getenv("SPTAG_FAULT_ONLINE_INSERT_IDEMPOTENCY_ON_RETRY") != nullptr) { /* armed */ }
//
// The harness sets SPTAG_FAULT_ONLINE_INSERT_IDEMPOTENCY_ON_RETRY=1 for the "fault" perf tier
