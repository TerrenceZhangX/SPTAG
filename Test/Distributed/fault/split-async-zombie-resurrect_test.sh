#!/usr/bin/env bash
# Per-case test harness: split-async-zombie-resurrect
#
# Tier-1 invariants for this case live entirely in
# Test/Distributed/fault/split-async-zombie-resurrect_test.cpp and
# exercise prim/op-id-idempotency + prim/ring-epoch-fence headers plus
# the env-gated split-async generation fence wrapper added in
...
#     SPTAG_FAULT_SPLIT_ASYNC_ZOMBIE_RESURRECT unset (env-off) and =1
#     (env-armed). Both invocations are scripted here.
#   * Tier 2 (1M perf triple): NOT on the hot-path strict list →
#     deferred per protocol; rationale recorded in
#     results/split-async-zombie-resurrect/STATUS.md.
#
# Usage:
#   ./split-async-zombie-resurrect_test.sh
#
# Env overrides:
#   BUILD_DIR  path to cmake build dir holding Release/SPTAGTest
#              default: $HOME/workspace/sptag-ft/build/split-async-zombie-resurrect

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
FAULT_SLUG="split-async-zombie-resurrect"
SUITE="SplitAsyncZombieResurrectTest"
...
# env-off
unset SPTAG_FAULT_SPLIT_ASYNC_ZOMBIE_RESURRECT
run_tier1 env-off

# env-armed
export SPTAG_FAULT_SPLIT_ASYNC_ZOMBIE_RESURRECT=1
run_tier1 env-armed
