#!/usr/bin/env bash
# Per-case test harness scaffold: online-insert-idempotency-on-retry
#
# Brings up PD + TiKV, runs the SPTAGTest cases for this fault slug,
# tears down. Multi-store / chaos sub-cases gated by --with-* flags.
#
# Usage:
#   ./online-insert-idempotency-on-retry_test.sh [--with-<subcase>] [...]
#
...
export FAULT_SLUG="online-insert-idempotency-on-retry"
...
echo "[harness] running SPTAGTest --run_test=OnlineInsertIdempotencyOnRetryTest"
"$TEST_BIN" --run_test=OnlineInsertIdempotencyOnRetryTest --log_level=test_suite --report_level=short
