...
namespace SPTAG::SPANN {
    ...
    public:
        ...
        void HandleAppendRequest(Socket::ConnectionID connID, Socket::Packet packet) {
            ...
            // Receiver-side ring-epoch fence (executes first: cheap stale reject).
            ...
            // Receiver-side idempotency dedup (after fence). Replay cached
            // status on hit; bump epoch tracking on a fresh restartEpoch.
            ...
            if (m_appendDedupCache && incomingOpId.IsValid()) {
                ...
                if (hit) {
                    SendAppendResponse(packet,
                        static_cast<RemoteAppendResponse::Status>(cached.status), local);
                    return;
                }
            }
